---
name: scrapling
description: "Adaptive web scraping framework with anti-bot bypass and spider crawling. Part of the web-scraping-tools umbrella."
...
```bash
# Core library (parser only) — ⚠️ THIẾU curl_cffi, không fetch được
pip install scrapling

# With fetchers (HTTP + browser automation) — ✅ BẮT BUỘC
...
**⚠️ Pitfall:** `pip install scrapling` thuần KHÔNG include `curl_cffi` → lỗi `ModuleNotFoundError: No module named 'curl_cffi'` khi import `Fetcher`. Luôn cài `scrapling[fetchers]`.

**✅ Verified:** Scrapling v0.4.9 trên Python 3.14.4 Windows — `Fetcher.get()` hoạt động, fetch httpbin trả về HTML content OK.
...
**⚠️ Pitfall:** `StealthyFetcher` dùng phương thức `fetch()`, **KHÔNG phải `get()`**:
```python
# ✅ Đúng
page = StealthyFetcher.fetch('https://example.com', headless=True)

# ❌ Sai — AttributeError: 'StealthyFetcher' object has no attribute 'get'
page = StealthyFetcher.get('https://example.com')
```
`StealthyFetcher` cần playwright/patchright browser riêng — không dùng headless Chrome thông thường.
...
### when to use what — Scrapling vs Pydoll

| Task | Scrapling | Pydoll |
|------|-----------|--------|
| Blog/news/docs, HTTP crawl | ✅ — nhanh, nhẹ, 0 token | ❌ overkill |
| JS-rendered pages (React SPA) | ❌ không render JS | ✅ CDP browser |
| Fill form, click, đặt vé | ❌ không interact | ✅ find(), click(), send_keys() |
| API reverse engineering | ✅ Fetcher.get() | ❌ không cần browser |
| Spider crawl nhiều trang | ✅ Spider class | ❌ manual loop |
| Login/auth flow | ❌ không hỗ trợ | ✅ browser tự nhiên |

**Rule:** Start with Scrapling. Nếu page trả về empty body hoặc 403 → fallback to Pydoll.
|---------|--------|-------|
...
- [[web-scraping-tools]] — **Umbrella skill** — So sánh Scrapling vs Pydoll vs Lightpanda vs browser-use
- [[js-page-extraction]] — Extract content từ JS-rendered SPAs (React/Angular/Vue)
...
### v1.0.11 (2026-06-11)
- **Thêm bảng so sánh Scrapling vs Pydoll** — khi nào dùng thư viện nào
- **Thêm rule:** Start with Scrapling, fallback Pydoll khi empty body / 403
- **Thêm execute_code note:** Import via `from hermes_tools import terminal` không dùng Scrapling trực tiếp từ execute_code — luôn chạy Scrapling trong terminal() hoặc script riêng

### v1.0.10 (2026-06-11)
- **Thêm pitfall:** `pip install scrapling` thuần thiếu `curl_cffi` — lỗi `ModuleNotFoundError`. Phải dùng `scrapling[fetchers]`
- **Thêm verified:** v0.4.9 test OK trên Python 3.14.4 Windows — Fetcher.get() hoạt động

### v1.0.9 (2026-06-11)
- **Cập nhật:** Liên kết tới skill umbrella `web-scraping-tools` trong Skill Graph section
