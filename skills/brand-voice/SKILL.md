...
## Project-Specific Voice Profiles

For the "Giải Mã Số Phận" project (Nhân tướng học & Mệnh lý học ứng dụng):
- Persona: Người đàn ông từng trải, gai góc, đã vấp ngã và đứng dậy — không phải "Quân Sư Khổng Minh"
- Tone: Trực tiếp, ngang hàng, không khuyên nhủ từ trên cao
- Full profile at [references/giai-ma-so-phan-brand-voice.md](references/giai-ma-so-phan-brand-voice.md)
...
## Brand Audit Workflow (Rebrand / Name Change)

When rebranding a project (renaming brand, persona, channel name), follow this systematic cleanup:

1. **Scan** — Search all brand-name variants in the project:
   - Full name with spaces: `search_files(pattern="Old Brand Name")`
   - Hashtag version: `search_files(pattern="OldBrandName")`
   - Lowercase/no-space variant: `search_files(pattern="oldbrand")`
   - Target: `content` with `files_only` output mode, limit=100
2. **Code scan** — Search `.py`, `.ts`, `.js`, `.json`, `.yaml`, `.html` files separately for hardcoded strings
3. **Fix** — Use `patch()` with `replace_all=True` for each file found
4. **Verify** — Re-scan all variants until count = 0
5. **Don't forget** — Build scripts, configs, template files, HTML/CSS, CI configs, README, social metadata
