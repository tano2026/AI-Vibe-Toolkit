...
- **Windows (bash) path with spaces:** When executing `terminal` commands on Windows (using bash shell), always enclose paths containing spaces in single quotes (e.g., `'C:\Users\Nguyen Ngoc Tan\...'`) to prevent "Is a directory" or "No such file or directory" errors.
