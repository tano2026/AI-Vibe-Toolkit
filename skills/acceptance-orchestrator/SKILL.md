...
2. **Issue gate** — If issue is not `ready`, stop immediately. Do not implement anything while issue remains `draft`.
3. **Execute** — Hand off for implementation and local verification.
4. **Review loop** — Batch PR feedback polling: wait `3m` → `6m` → `10m`. After the `10m` round, stop waiting and process all visible comments together.
5. **Deploy and runtime verification** — If DoD depends on runtime behavior, deploy only to `dev` by default. Verify with real logs/API/Lambda behavior, not assumptions.
6. **Completion gate** — Before any claim of completion, require verification with fresh evidence.
